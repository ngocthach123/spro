...
class Modelcontentnews extends Model {
	public function addnews($data) {
        ...
        if(isset($data['downloads'])) {
		   // echo "UPDATE " . DB_PREFIX .news SET filename = '" . $this->db->escape($data['downloads']) . "'WHERE news_id = '" . (int)$news_id ."'";exit;
		   $this->db->query("UPDATE " . DB_PREFIX . "news SET filename = '" . $this->db->escape($data['downloads']) . "'WHERE news_id = '" . (int)$news_id ."'");
        }
	...
	public function editnews($news_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "news SET  date_available = '" . $this->db->escape($data['date_available']) . "', status = '" . (int)$data['status'] . "', airline = '" . (int)$data['code_airline']. "', departure = '" . (int)$data['departure'] . "', sort_order = '" . (int)$data['sort_order'] . "', date_added = NOW(), date_modified = NOW() WHERE news_id = '" . (int)$news_id . "'");
    ...
    public function getairline() {
        $query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "airline ai");
                
        return $query->rows;
    }	
    
    public function getTotalairline($airline_id) {
        $query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "airflights WHERE airline_id = '" . (int)$airline_id . "'");

        return $query->row['total'];
    }    
    
    public function getairflightByairline($airline_id) {
        $query = $this->db->query("SELECT a.code,a.name,a.airflights_id FROM " . DB_PREFIX . "airflights a WHERE a.airline_id = '" . (int)$airline_id . "' ORDER BY a.name ASC");
        return $query->rows;       
    } 
}
?>