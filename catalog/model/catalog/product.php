...
class ModelCatalogProduct extends Model {
	...
	public function getProductAccessByGroup($group_id) {
		$product_access_data = array();
	...
	public function getProduct($product_id) {
		...
		$query_2 = $this->db->query("SELECT COUNT(*) AS num_rate, r1.rating FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = ".(int)$product_id." AND r1.status = '1' GROUP BY r1.rating ORDER BY r1.rating DESC");
		if ($query->num_rows) {
			return array(
				'product_id'       => $query->row['product_id'],
				'name'             => $query->row['name'],
				'description'      => $query->row['description'],
				'specs'      => $query->row['specs'],
				'bonus'      => $query->row['bonus'],
	...
	public function getProductOptions($product_id, $default=0) {
		...
		if($default){
			$product_option_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_option po LEFT JOIN `" . DB_PREFIX . "option` o ON (po.option_id = o.option_id) LEFT JOIN " . DB_PREFIX . "option_description od ON (o.option_id = od.option_id) WHERE po.product_id = '0' AND od.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY o.sort_order");
		}else{
