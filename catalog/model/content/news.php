...
class Modelcontentnews extends Model {
	...
	public function getCategories($news_id) {
		...
		return $query->rows;
         ...
         public function getcategorynewrelated($new_id) {
          if ($this->customer->isLogged()) {
                $customer_group_id = $this->customer->getCustomerGroupId();
            } else {
                $customer_group_id = $this->config->get('config_customer_group_id');
            }    
            $query=$this->db->query("SELECT cat_id FROM ". DB_PREFIX ."news_to_cat c where c.news_id= '" . (int)$new_id . "'");
...
return $query->row['total'];
	...
	public function getairfight($airflights_id) {
        $query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "airflights ai WHERE ai.airflights_id = '" . (int)$airflights_id . "'");
