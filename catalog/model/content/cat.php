...
class Modelcontentcat extends Model {
	...
	public function getCategories($parent_id = 0) {
	   //echo "SELECT * FROM " . DB_PREFIX . "cat c LEFT JOIN " . DB_PREFIX . "cat_description cd ON (c.cat_id = cd.cat_id) LEFT JOIN " . DB_PREFIX . "cat_to_store c2s ON (c.cat_id = c2s.cat_id) WHERE c.parent_id = '" . (int)$parent_id . "' AND cd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND c2s.store_id = '" . (int)$this->config->get('config_store_id') . "'  AND c.status = '1' ORDER BY c.sort_order, LCASE(cd.name)";exit;
       $sql = "SELECT * FROM " . DB_PREFIX . "cat c LEFT JOIN " . DB_PREFIX . "cat_description cd ON (c.cat_id = cd.cat_id) LEFT JOIN " . DB_PREFIX . "cat_to_store c2s ON (c.cat_id = c2s.cat_id) WHERE c.parent_id = '" . (int)$parent_id . "' AND cd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND c2s.store_id = '" . (int)$this->config->get('config_store_id') . "'  AND c.status = '1' ORDER BY c.sort_order, LCASE(cd.name)";
